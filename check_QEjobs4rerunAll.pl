...
print "\n\nMaking fake dead.txt in QEjobs_status for rerun all jobs\n";

my @donejobs = `cat QEjobs_status/Done.txt|grep -v '#'`;
map { s/^\s+|\s+$//g; } @donejobs;
for my $d (@donejobs){
    print "Done job to be fake dead one: $d\n";
    `echo "0/0: $d" >> QEjobs_status/Dead.txt`;
    #$temp =~ s/^\s+|\s+$//g;
    #if(!$temp){
    #    print "Adding $d to fake dead.txt\n";
    #    open(my $FAKE, ">> QEjobs_status/fake_dead.txt") or die $!;
    #    print $FAKE "$d\n";
    #    close($FAKE);
    #}
}  
